...
> This repository is the **common plugins layer** for the Moku family (`core`, `web`, `blog`, …).
> Plugins authored here are intended to be reused across sibling Moku projects rather than
...
This package is a **plugin catalog**, not a framework. It does not call
`createCoreConfig` / `createCore` and ships no `createApp` — it only exports plugin
objects (plus their providers and `Log`/`Env` type namespaces) built on
`@moku-labs/core`. Consuming frameworks (e.g. `@moku-labs/web`) import these plugins
and register them in their own `createCoreConfig`.

- `src/plugins/<name>/` — one shared plugin each. These are **core plugins**
  (`createCorePlugin` from `@moku-labs/core`), so their API is injected onto every
  plugin's context (`ctx.log`, `ctx.env`) and they carry no framework binding.
- `src/index.ts` — the catalog barrel: the full surface, incl. the Node env providers.
- `src/browser.ts` — the node-free subset for client bundles (built as its own pass).

## Testing

- Vitest with unit + integration projects
- Catalog-level tests: `tests/unit/` and `tests/integration/` (barrel smoke tests, cross-plugin scenarios)
- Plugin-specific tests: `src/plugins/[name]/__tests__/unit/` and `__tests__/integration/` (colocated inside each plugin)
- 90% coverage threshold
- Never put plugin-specific tests in root `tests/` — root tests are for catalog-level integration only
