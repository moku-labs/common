#!/usr/bin/env bun
/**
 * @file Post-build gate for the `@moku-labs/common/browser` entry. Run with `bun`.
 *
 ...
 *
 *   1. no-node-leak — no STATIC import of `node:*` anywhere in the entry's
 *      static-import closure (the only node concern in this package is the env
 *      plugin's `dotenv`/`processEnv`/`cloudflareBindings` providers, which import
 *      `node:fs` and must stay on the `.` entry only);
 *   2. size-budget — the static closure (summed per-file gzip) stays under
 ...
 *
 * Runnable locally and in CI via `bun run check:bundle`. Node built-ins only — no
 * dependencies.
 */
...
const BANNED_SPECIFIERS: readonly RegExp[] = [/^node:/];

/** Budget for the static closure, measured as the sum of each file's gzip size (this entry is small — log + env + browserEnv). */
...
function analyzeBrowserBundle(): GateReport {
  ...
  return {
    ...
    nodeEntry: existsSync(NODE_ENTRY)
      ...
      : undefined,
    problems: [...findNodeLeaks(closure), ...checkSizeBudget(closureGzipBytes)]
  };
}
