...
Register `log` as a core plugin in your framework's `createCoreConfig` (Layer 2). Its API is then injected as `ctx.log` on every plugin and surfaced as `app.log`.

```ts
import { createCoreConfig } from "@moku-labs/core";
import { logPlugin } from "@moku-labs/common";

export const coreConfig = createCoreConfig<Config, Events, [typeof logPlugin]>("my-framework", {
  config: {},
  plugins: [logPlugin],
  pluginConfigs: { log: { mode: "test" } } // trace only, no console noise
});
```

```ts
// Inside any plugin use `ctx.log`; from the composed app use `app.log`:
...
`mode` resolves via the 4-level core-plugin cascade: spec default (`"production"`) → `createCoreConfig` → `createCore` → `createApp`. A consuming framework's `createCoreConfig` typically pins the level-2 default (e.g. `log: { mode: "production" }`); consumers and tests override it through `createApp({ pluginConfigs: { log: { mode: ... } } })` (e.g. `"test"` or `"silent"` to silence console output). There is **no automatic mapping** from any global `Config` field onto `log.mode`.
