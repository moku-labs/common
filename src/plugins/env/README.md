...
Register `env` as a core plugin in your framework's `createCoreConfig`, and supply the providers that match your target.

```ts
import { createCoreConfig } from "@moku-labs/core";
import { dotenv, envPlugin, processEnv } from "@moku-labs/common";

export const coreConfig = createCoreConfig<Config, Events, [typeof envPlugin]>("my-framework", {
  config: {},
  plugins: [envPlugin],
  pluginConfigs: {
    env: {
...
```ts
// `ctx.env` on every plugin, `app.env` on the composed app:
...
| `require` | `(key: string) => string` | Resolved value, or **throws** `[<framework>] env: required variable "<key>" is not defined.` if undefined (the prefix is the consuming framework's id). |
...
> Cloudflare deployments typically use `[cloudflareBindings(), processEnv()]`. For the
> browser, import `browserEnv` from `@moku-labs/common/browser` and wire it as the provider
> (a consuming framework's own browser entry may pre-wire it so the browser needs zero config).
